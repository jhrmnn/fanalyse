...
def parse_source(filename: str, preprocessor: str = None) -> Any:
    try:
        if preprocessor:
            source = subprocess.run(
                f'{preprocessor} {filename}', shell=True, capture_output=True
            ).stdout.decode()
        else:
...
def parse(filenames: List[str], jobs: int = None, preprocessor: str = None) -> None:
    if len(filenames) == 1:
        parse_source(filenames[0], preprocessor)
        return
    parsed = Collector(len(filenames))
    pool = Pool(jobs)
    for filename in filenames:
        pool.apply_async(parse_source, (filename, preprocessor), callback=parsed.update)
    pool.close()
    pool.join()
