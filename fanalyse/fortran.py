...
def _uu(iterable):
    return reduce(
        or_,
        (getattr(x, '_used', set()) for x in iterable),
        set()
    ) if iterable else set()


def _u(x):
    return getattr(x, '_used', set())
...
classes = [
    ...
    ('DoBlock', lambda self: _u(self.condition) | _u(self.from_) |
     _u(self.to) | _u(self.by) | _uu(self.body)),
    ...
    ('WhereBlock', lambda self: _u(self.where) | _uu(self.body) |
     _uu(self.elsebody)),
    ...
    ('Iterator', lambda self: _u(self.from_) | _u(self.to) | _u(self.by) |
     _uu(self.values)),
